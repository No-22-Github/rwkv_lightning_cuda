...
另外，只要目标标签 ref 已存在，任何对草稿的 PATCH 都会使其 `tag_name` 被改写为
占位标签（创建标签后的发布说明更新即可触发）。因此发布器会在发布翻转前重新读取
并重钉真实标签，发布后再次校验绑定；若无法钉回则直接令任务失败，
而不是把 Release 留在占位标签上、让下一次推送重复发布同一版本。
